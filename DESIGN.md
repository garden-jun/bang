...
- **폴링 간격**: 대기실 3초 / 게임 중 2초 / 지금 행동할 사람만 1초. 안 보고 있는 탭은 8초.
...
아무것도 못 보고 지나간다. 사건을 큐에 쌓아 0.9초 간격으로 하나씩 흘리고(밀리면
간격을 줄여 따라잡는다), 피해·회복·사망은 좌석 흔들림/섬광으로 표시한다.

서버 쪽에서도 봇의 한 수가 최소 `BOT_MIN_MOVE_MS`(기본 1100)는 걸리게 하고,
봇 차례의 `pollMs`를 1200으로 맞춰 수가 뭉쳐 오지 않게 한다.
...
- `useGameEvents`가 메타를 화살표 사건으로 바꿔 기존 큐(0.9초 간격)에 태운다. 봇이 연달아
