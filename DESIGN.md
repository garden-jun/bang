...
- **폴링 간격**: 대기실 1.2초 / 게임 중 0.8초 / 지금 행동할 사람 0.7초 / 봇 차례 1.8초. 안 보고 있는 탭은 8초.
...
`join`의 `as`는 `player` / `spectator` / **`auto`**. `auto`는 자리 여부를 **락 안에서** 보고
정한다 (대기 중 + 자리 있음이면 player, 아니면 spectator. 이미 앉아 있으면 그 자리 유지).
예전에는 클라이언트가 `roomInfo`를 먼저 읽어 골랐는데, 그 왕복이 입장 경로에 직렬로 얹혀
**남의 화면에 내가 뜨는 시간을 늦췄고**(실측 0.23초) 값도 이미 낡은 것이었다 — 마지막 한
자리에 둘이 동시에 들어오면 한 명은 관전으로 내려가지 않고 "가득 찼습니다"로 튕겼다.
`roomInfo`는 이제 입장 전 안내 화면 표시용으로만 쓰이고 입장과 나란히 간다.
검증: `auto-join.test.ts` (동시 입장 포함).
...
서버 쪽에서는 **봇이 직전 수의 연출이 화면에서 끝날 때까지 기다린다** (15절). 봇 차례의
`pollMs`는 1800으로 맞춰 수가 뭉쳐 오지 않게 한다. 간격 숫자(`STEP_MS` 등)는
`shared/pacing.ts` 한곳에 두고 화면과 서버가 같이 읽는다. CSS 연출 길이도 같이 움직여야 한다.
...
| 층 | 무엇 | 어디 |
|---|---|---|
| ① 시작 안내 | 내 역할·목표, 역할 4개 요약, 턴 순서, 거리 규칙. 카드/캐릭터 전체 목록 탭 | 판 시작 시 한 번 오버레이(`HelpSheet`), 헤더 `도움말`로 재열람 |
...
- 헤더 `?` 토글(localStorage). 켜져 있으면 강조색. 끄면 ②③이 사라지고 한 번 탭으로 돌아간다. ①은 켜져 있을 때만 판마다 한 번.
...
## 14. 감옥·다이너마이트 (턴 시작 판정)

감옥과 다이너마이트가 좌석 장착 줄의 작은 카드로만 보였고, 판정은 서버가 자동으로 넘겨
"감옥 판정: 레밍턴 ♣K → 실패" 한 줄이 말풍선으로 지나갈 뿐이었다. 누가 갇혔는지,
무엇이 나와서 왜 실패했는지, 다이너마이트가 어디로 갔는지를 따라갈 수 없었다.

- **들고 있는 동안**: 감옥은 좌석 위에 창살을 겹치고 왼쪽 위에 `감옥` 배지, 다이너마이트는
  오른쪽 위에 불꽃이 깜빡이는 배지. 장착 줄 카드는 그대로 둔다(올리면 설명).
- **판정 순간**: 엔진이 `drawCheck`에서 `{ kind: "check", check, cards, ok }` 메타를 남기고,
  화면은 테이블 중앙에 카드를 뒤집어 보여주며 결과 도장을 찍는다. 술통 판정도 같은 함수라
  같이 보인다. 엔진의 `ok`는 "조건이 나왔다"라서 다이너마이트는 `ok`가 곧 폭발 — 도장은
  그 사람 입장의 말(탈출/갇힘, 불발/폭발, 막음/실패)로 바꿔 쓴다. 럭키 듀크는 두 장을 보여주고
  결정한 장을 짚는다. 좌석이 중앙까지 차는 폰에서도 보이도록 좌석 위(z-30)에 띄운다.
- **결과**: `{ kind: "outcome", outcome: "escape" | "skip" | "explode" }` → 좌석 배지
  (탈출은 창살이 떨어져 나감, 폭발은 섬광). 불발은 `{ kind: "dynamite", from, to }` 화살표.
- 판정 장면은 큐가 밀려도 `CHECK_HOLD_MS`(1.5초) 붙잡는다. 내 차례 따라잡기 중에는 내 판정일 때만.
- **배지는 흔들림과 따로 둔다** (`SeatBadge` vs `SeatEffect`). 폭발 배지 바로 뒤에 "피해 3"
  흔들림이 오는데, 좌석 상자는 흔들림마다 key로 갈아끼워져 그 안의 일회성 연출이 처음부터
  다시 재생됐다. 좌석 바깥을 고정 틀로 두고 배지는 틀에, 흔들림은 안쪽 상자에 건다.

**로그 번호 버그**: `t`를 `log.length`로 매기고 있었는데 로그는 80줄에서 앞을 잘라낸다. 그 뒤로
모든 줄이 `t = 80`이 되어, "마지막 본 번호보다 큰 줄"을 새 사건으로 보는 화면이 아무것도
알아채지 못했다 — 게임 중반부터 화살표·흔들림·말풍선이 통째로 멈췄다. 직전 줄 번호 + 1로 매긴다.

## 15. 봇은 연출이 끝난 뒤에 둔다

봇의 한 수마다 1.65초(`BOT_MIN_MOVE_MS`)만 쉬었는데, 한 수가 로그를 여러 줄 남기면(뽑기·공격·
술통 판정·피해) 화면은 그걸 한 줄씩 흘리느라 5초 넘게 걸렸다. 봇이 연출을 계속 앞질러, 앞 사람의
장면이 끝나기도 전에 다음 봇이 움직였다.

- **서버가 연출 길이를 계산한다.** 상태를 바꾼 뒤 새로 생긴 로그 줄을 화면의 pump와 같은 규칙
  (`shared/pacing.ts`의 `playbackMs` — 남은 개수에 따라 줄어드는 간격, 판정은 1.5초 붙잡기)으로
  재고, 폴링 도착 여유 0.7초를 더해 `room.botNotBefore`에 적는다. 최소는 여전히 `BOT_MIN_MOVE_MS`.
- **사람의 수 뒤에도 건다** (`doAction`, 시간초과 처리, 게임 시작). 사람이 뱅!을 쏘자마자 봇이
  대응하면 쏘는 장면과 막는 장면이 겹친다.
- **기다림은 구동부 안이 아니라 방에 적는다.** 구동부는 액션·폴링 요청마다 따로 뜨므로, 한
  구동부 안에서만 쉬면 다른 요청이 띄운 구동부가 바로 둬 버린다.
- 구동부는 기다림이 8초 예산 안이면 자고, 넘치면 물러난다. 그 뒤로 방이 안 바뀌면 폴링이 전부
  304라 아무도 구동부를 다시 띄우지 않으므로, **304에서도 `botDue`(봇 차례 + 기다림 끝)면 띄운다**
  (`pollState`). 기다림이 안 끝났으면 띄우지 않는다 — 이미 기다리는 구동부가 있다.
- 테스트: `bot-pacing.test.ts`. `BOT_MIN_MOVE_MS=0`이면 기다림이 통째로 꺼진다(다른 봇 테스트).

**좌석 흔들림**은 한 번도 나오지 않고 있었다. `anim-hit`(흔들림)과 `anim-flash-hit`(섬광)을 한
요소에 걸었는데 둘 다 `animation` 속성이라 뒤의 섬광이 흔들림을 덮어썼고, 차례인 좌석은
`anim-active`가 둘 다 덮었다. 흔들림·섬광은 한 선언으로 합치고, 좌석 상자(`anim-active`)와
다른 요소에 건다.
