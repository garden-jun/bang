...
| 키 | 타입 | TTL | 내용 |
|---|---|---|---|
| `room:{code}` | string(JSON) | 6h (활동 시 갱신) | `RoomState` 전체 (대기실 정보 + 게임 상태). 접속 중인 사람이 10분간 없으면 로비 조회 때 삭제 |
...
기준은 "사람 수"가 아니라 **마지막으로 폴링한 사람의 시각**이다. 사람 1명이 봇들과
플레이 중인 방은 지우면 안 되는데, 그 사람이 보고 있는 한 `lastSeen`이 계속 갱신되므로
걸리지 않는다. 봇은 폴링하지 않으므로 애초에 세지 않는다.
